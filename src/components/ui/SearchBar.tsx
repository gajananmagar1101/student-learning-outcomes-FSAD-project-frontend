  ...
  useEffect(() => {
    ...
    const loadSearchData = async () => {
      try {
        ...
        if (isStaffRole(user?.role) && students.length === 0) {
          await fetchStudents()
        }
        if (isStaffRole(user?.role) && adminAssignments.length === 0) {
          await fetchAdminAssignments()
        }
        if (user?.role === 'student' && studentAssignments.length === 0) {
          await fetchStudentAssignments()
        }
        if (user?.role === 'admin' && submissions.length === 0) {
        ...
        // keep search usable with whatever data is already loaded
      }
      ...
      if (user?.role === 'admin') {
        try {
          const facultyResponse = await facultyRequestAPI.getFaculty()
          if (active) {
